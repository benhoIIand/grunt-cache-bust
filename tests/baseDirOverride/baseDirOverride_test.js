...
module.exports = {
    ...
    baseDirOverride: function(test) {
        ...
        test.ok(grunt.file.exists('tmp/assets/others/' + markup.match(/baseDirOverride\.[a-z0-9]{16}\.js/)[0]));
        test.ok(markup.match(/baseDirOverride\.[a-z0-9]{16}\.css/), 'testing baseDirOverride combination of CSS, JS and images');
        test.ok(grunt.file.exists('tmp/assets/others/' + markup.match(/baseDirOverride\.[a-z0-9]{16}\.css/)[0]));
        test.ok(markup.match(/baseDirOverride\.[a-z0-9]{16}\.png/), 'testing baseDirOverride combination of CSS, JS and images');
        test.ok(grunt.file.exists('tmp/assets/others/' + markup.match(/baseDirOverride\.[a-z0-9]{16}\.png/)[0]));
